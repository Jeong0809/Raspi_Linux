...
int main(int argc, char** argv) {
	...
	for(i=0; i<height*3; i+=3) { 
		for(j=0; j<width*3; j+=3) {
			outimg[(j*xFactor)+(width*xFactor*i*yFactor)]=
											inimg[j+(i*width)]; 
			outimg[(j*xFactor/2)+(width*xFactor*i*yFactor)]=
											inimg[j+1/2+(i*width)];

			outimg[(j*xFactor)+(width*xFactor*i*yFactor)+1]=
											inimg[j+(i*width)+1];
			outimg[(j*xFactor/2)+(width*xFactor*i*yFactor)+1]=
											inimg[j+1/2+(i*width)+1];

			outimg[(j*xFactor)+(width*xFactor*i*yFactor)+2]=
											inimg[j+(i*width)+2];
			outimg[(j*xFactor/2)+(width*xFactor*i*yFactor)+2]=
											inimg[j+1/2+(i*width)+2];
