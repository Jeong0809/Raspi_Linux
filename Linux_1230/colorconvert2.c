...
int main(int argc, char **argv)
{
    ...
    pBmpData = (ubyte *)malloc(vinfo.xres_virtual * vinfo.yres_virtual * sizeof(ubyte) * vinfo.bits_per_pixel/8);
    pData = (ubyte*)malloc(vinfo.xres_virtual * vinfo.yres_virtual * sizeof(ubyte) * color/8);
    pImageData = (ubyte *)malloc(vinfo.xres_virtual * vinfo.yres_virtual * sizeof(ubyte) * color/8);

    /* 프레임 버퍼에 대한 메모리 맵을 수행한다. */
    pFbMap = (ubyte *)mmap(0, vinfo.xres_virtual * vinfo.yres_virtual * vinfo.bits_per_pixel/8, PROT_READ|PROT_WRITE, MAP_SHARED, fbfd, 0);
    ...
    cvtBGR2Gray(pData, pImageData, vinfo.xres_virtual*vinfo.yres_virtual);

    /* BMP 이미지 데이터를 프레임 버퍼 데이터로 변경 */
    for(int y = 0, k, total_y; y < rows; y++) {
        k = (rows-y-1)*cols*color/8;
        total_y = y*vinfo.xres_virtual*vinfo.bits_per_pixel/8;
    ...
    memcpy(pFbMap, pBmpData, vinfo.xres_virtual*vinfo.yres_virtual*vinfo.bits_per_pixel/8);

    /* 프레임 버퍼 파일을 닫고 이미지 데이터를 위해서 사용한 메모리를 해제 */
    munmap(pFbMap, vinfo.xres_virtual*vinfo.yres_virtual*vinfo.bits_per_pixel/8);
