...
void thres_lines(Mat acc_dst, Mat& lines, double _tho, double theta, int thresh){
	for(int r = 0; r < acc_dst.rows; r++){
		for(int t = 0; t < acc_dst.cols; t++){
			float value = (float)acc_dst.at<int>(r, t);
			if(value >=  thresh){
				float rho = (float)((r - acc_dst.rows / 2) * _tho);
...
int main()
{
	Mat image = imread("../Linux_1219/line1.bmp", 0);
