...
int main(int argc, char** argv) 
{
    ...
    int x, y, z, imageSize, outimageSize, paddingSize;
	...
	paddingSize = ((bmpInfoHeader.biWidth + bmpInfoHeader.biHeight +2)*3) *2;

    /* 이미지의 해상도(넓이 × 깊이) */
    printf("Resolution : %d x %d\n", bmpInfoHeader.biWidth, bmpInfoHeader.biHeight);
    printf("Bit Count : %d\n", bmpInfoHeader.biBitCount);     /* 픽셀당 비트 수(색상) */
    printf("Image Size : %d\n", imageSize);
    printf("padding Image Size : %d\n", paddingSize+imageSize);

    inimg = (ubyte*)malloc(sizeof(ubyte)*imageSize); 
    outimg = (ubyte*)malloc(sizeof(ubyte)*imageSize);
	paddingimg = (ubyte*)malloc(sizeof(ubyte)*(imageSize + paddingSize));
    ...
    for(y = 1; y < bmpInfoHeader.biHeight +1; y++) { 
        for(x = 1; x < (bmpInfoHeader.biWidth +1) * elemSize; x+=elemSize) {
            for(z = 0; z < elemSize; z++) {
                ...
                for(int i = -1; i < 2; i++) {
                    for(int j = -1; j < 2; j++) {
                        sum += kernel[i+1][j+1]*paddingimg[(x-i*elemSize)
												+(y-j)*size+z];
                    }
                }
